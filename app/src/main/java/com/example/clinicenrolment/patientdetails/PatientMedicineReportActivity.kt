...
class PatientMedicineReportActivity : AppCompatActivity() , View.OnClickListener {
    ...
    private var reasonObject :Reason?=null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_patient_medicine_report_layout)


        patientObject  = intent.getSerializableExtra("patientObject") as? PatientDetail

        reasonObject = intent.getSerializableExtra("reasonObject") as? Reason

        Log.e("GOt POJO ","Pateint detail"+patientObject?.apptDate)

        Log.e("REASON","Data"+reasonObject?.reasontoMeet)
    ...
    private fun initViews() {
        ...
        var toolbarSetup = ToolbarSetup(this,toolbar,"Medicine",R.drawable.ic_backicon)
                            ...
                            // for(medicinedata :DataSnapshot in dataList.children)
                             //{
                                 ...
                                 if(medicinePojo?.patientId!!.equals(patientObject.pateintId)
                                     && medicinePojo?.reasodId!!.equals(reasonObject!!.reasonId))
                                 {
    ...
    private fun getReasonList() {
        ...
        val resonList = object: ValueEventListener {
            ...
            override fun onDataChange(data: DataSnapshot) {
                ...
                {
                    linearLayoutManager = LinearLayoutManager(this@PatientMedicineReportActivity)
                    reasonRecyclerView?.layoutManager = linearLayoutManager

                    reasonRecyclerView?.addItemDecoration(DividerItemDecoration(this@PatientMedicineReportActivity,DividerItemDecoration.VERTICAL))

                    //  patientDataRecyclerView.adapter = PatientListAdapter(patientDataList,activity)

                    reasonAdapter = ReasonListAdapter(reasonDataList,{ resonItem: Reason-> onItemClicked(resonItem) },this@PatientMedicineReportActivity )
    ...
    private fun setupDataList(medicinelist: ArrayList<Medicine>) {
        ...
        if(medicinelist!=null)
        {
            ...
            medicineRecyclerView?.addItemDecoration(DividerItemDecoration(this@PatientMedicineReportActivity,DividerItemDecoration.VERTICAL))

            //  patientDataRecyclerView.adapter = PatientListAdapter(patientDataList,activity)

            medicineAdapter = PatientMedicineListAdapter(medicinelist,this@PatientMedicineReportActivity )
